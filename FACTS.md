...
| 1 agent | What the free trial runs, and what an account with no plan runs | Archie repo, `crates/archie-core/src/plan.rs`, `FREE_AGENTS`. Enforced in `agent_create` before the row is written. Reads a licence of `none` as well as a trial, so it is also the free tier's agent count |
