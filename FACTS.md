...
| `$25` | AI usage included each month on that plan, at Anthropic's published prices, on Claude Sonnet, refilled each paid month with nothing carried over | Archie repo, `PLAN_GRANT_MICROS` on the billing service (25,000,000 micro-dollars) and `PLAN_AI_ALLOWANCE` in `src/app/pricing.ts`. Against `docs/COST-MEASURED.md`: a light month fits several times, a moderate month is about half, a heavy month does not fit |
...
| Figure | What it is | Where it comes from |
|---|---|---|
| 135 add-ons | 36 personalities, 58 skills, 3 specialists, 38 routines: what a visitor can install | Archie repo `data/marketplace/**`, counted 2026-09-01 by `check-facts.py`, which skips any manifest marked `visibility: private`. **The convention changed that day, on Jett's call.** Every printed count had been the whole directory, which was 153 and included 18 private add-ons: one client's sales pack and two items for a beta account, none of which any visitor can reach. A number on a public page describes what a reader can go and get, so a private add-on is not in it. The directory total is still worth knowing and is not printed anywhere. History: 151 on 2026-08-31 (`routines/evening-journal.json` retired 2026-08-28 after a live agent fired it and Evening Reflection half an hour apart; `routines/statement-round.json` landed 2026-08-30), and 153 on 2026-09-01 under the old convention. **The down move is why the check exists and the up move is why it has to run on every commit**: a deletion nobody thinks of as a stat change sat wrong for a day, and an addition did the same. |
