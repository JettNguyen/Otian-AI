...
| About 15 minutes, once | The setup estimate on the homepage: what the five steps add up to for one person on one computer, and the clock that counts to it above the track, which reaches 1, 4, 9, 13 and 15 as each step is lit | **An estimate, and the site says so in the same breath**: the readout is labelled "Estimated", the heading says "about fifteen minutes", and the line under the track opens on "Estimates, not a stopwatch." That is the whole of what the page says about it, on Jett's call of 2026-09-08. It had also carried "Nobody has timed a clean install", and that sentence went: the label already tells a reader what kind of number this is, and spelling out that we have not measured our own installer is the self-criticism the Standard says to delete when the sentence is true and complete without it. What we have not measured is recorded here instead, which is where it belongs. It is built from the five steps rather than picked as a total: about a minute to settle on a computer, three to download and install (the release assets are 16 MB on Windows and 48 MB on a Mac, plus an OS permission prompt), five to open an account with an AI company and put a card on it, four to answer the setup questions, and two for the claim code that connects a chat app. **The middle one is the longest and it is not ours**, which is why the page names it rather than burying it in the total: Archie cannot make an account with Anthropic on somebody's behalf, and how long that takes is that company's checkout, not our installer. Written up 2026-09-08 on Jett's direction, after a competitor teardown found "60 seconds" on one of their pages and "2 minutes" on three others. Deliberately not rounded down and deliberately not best-case: the Standard's rule about third-party costs is that they get realistic estimates, and this is a cost in time rather than money. **The numbers live in one list**: the five cumulative minutes are what the clock counts to and what every fill and highlight window in the track is generated from, so a timed install changes the heading, that list, and this row, and nothing else moves. They are not printed on the steps themselves and must not be: a running total beside step five reads as the time step five takes, which is the opposite of what it means |
